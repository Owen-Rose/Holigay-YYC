...
Specs 005 and 006 shipped to `dev` together in PR #7 (2026-08-22) — one 13-commit fast-forward; their histories were inseparable. Migration `011` is applied to **both** Supabase projects: dev on 2026-08-22, prod on 2026-09-13 (`009`–`011` pushed together after repairing prod's empty migration history), and `dev` was promoted to `main` the same day so Vercel Production runs the matching code. The manual probe checklist and a live test submission on prod are still owed — see the "Prod rollout record" in `specs/006-close-public-data-exposure/quickstart.md`. Spec 005 is closed with its T050 manual walkthrough **waived, not executed** (reasoning in its `tasks.md`); its Phase 11 (2026-09-14, branch `005-atomic-builder-save`, PR to `dev`) closed the two Tier 2 defects — the builder and the template seed now write through one atomic `save_event_questionnaire` RPC (migration `012`) and `seeded_from_template_id` is populated — with real-database suites for the builder/template write paths that retire the waiver's main gaps. Migration `012` is applied to local, dev (2026-09-14) and prod (2026-09-14); the dev-preview organizer probe passed and `dev` was promoted to `main` the same day — see the "Phase 11 rollout" table in `specs/005-dynamic-questionnaires/quickstart.md`. Specs 001 / 002 / 004 are merged; all `docs/cleanup-roadmap.md` workstreams are complete (that file is historical — current planning lives in `docs/ROADMAP.md`). See `specs/README.md` for an at-a-glance status of every spec.
...
## Recent Changes
- 008-self-hosted-infrastructure (designed 2026-09-19, plan/tasks pending): move off Vercel, hosted Supabase and the Resend SDK onto a Raspberry Pi 5 (prod) and a desktop (staging/backup) running the trimmed Supabase stack (Postgres + GoTrue + PostgREST + storage-api) behind Caddy, single origin per environment, restic backups, Cloudflare-proxied DNS, WireGuard, Ansible. No application rewrite; `APP_ENV` replaces `VERCEL_ENV`, nodemailer replaces the Resend SDK. Design record: `specs/008-self-hosted-infrastructure/research.md`.
- 007-production-readiness (planned 2026-09-14, in progress): M3 as one spec — env contract keyed on `VERCEL_ENV` (production-only strictness, `resend.dev` refused), email-warning parity for the dynamic form, daily keep-alive cron for both Supabase projects, `npm run smoke` + event-week and backup/restore runbooks, and a solo lifecycle rehearsal on the dev preview as the gate. Manual dashboard work is `[manual]` tasks with evidence in its `quickstart.md`. No schema, RLS or auth changes.
- 005 Phase 11 (atomic questionnaire save): `saveEventQuestionnaire` replaces the four per-question actions; the builder assigns question ids client-side and saves in one `save_event_questionnaire` call (migration 012); the template seed uses the same RPC and finally records `seeded_from_template_id`; `src/test/security/questionnaire-save.test.ts` and `template-writes.test.ts` add real-database coverage for the builder and template write paths.
