...
- Modify: `src/lib/env.ts`, `src/test/env.test.ts`, `.env.example`, `package.json`, `docs/runbooks/event-week-smoke.md`, `docs/ARCHITECTURE.md`, `docs/ROADMAP.md`, `CLAUDE.md`, `README.md`, `docs/DEV-ENVIRONMENT-SETUP.md`, `specs/007-production-readiness/contracts/env-contract.md`, `specs/README.md`, `.specify/memory/constitution.md`
...
`.specify/memory/constitution.md` Principle I: change "is generated by `npm run db:types` / `db:types:dev`" to "is generated by `npm run db:types` from the local stack (spec 008)" — the Governance clause requires the constitution's text to follow the repository in the same PR.
...
- **If T003 (sending-domain verification) is slow**: T017 needs the code on `main`, not a verified sender. Promote with `EMAIL_FROM_ADDRESS` set to the intended address on the real domain (the env contract refuses only the test domain; delivery is not needed until T018's click-through), or temporarily set the `production` GitHub environment's variables and push a `main` snapshot of `dev` — either way, record what was done in the T012 evidence row.
