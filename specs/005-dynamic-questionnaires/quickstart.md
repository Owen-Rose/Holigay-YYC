...
| dev | 2026-09-14 (`db push`, only 012 pending; history intact) | anon-key probe: `save_event_questionnaire` present and `42501` for anon, both older RPCs `42501`, public event read OK; `migration list --linked` shows 012; types from dev match the committed file apart from the generator's cosmetic block | Organizer-side builder probe run 2026-09-14 on the `dev` preview (commit `0ee5921`, deployment `holigay-nksu70cuz`) against event `3c3a7676-cabd-4ca0-9db6-dba483a75a0b` ("Phase 11 probe 2026-09-14", throwaway draft): added a yes/no question and a short-text question showing-if the first equals Yes, moved the trigger above it, removed the default "Special Requirements" question, saved once → exactly one POST to the event page, "Questionnaire saved" toast, full state (order, deletion, show-if) present after a hard reload. Seeded from template "test" → one POST, "Questions added from template" toast, the appended question present after reload. `event_questionnaires.seeded_from_template_id` for that event = `65770fa4-c72d-4ed3-b6f0-1833a1044289` (updated 2026-09-14 02:40 UTC), confirmed in the dev SQL editor — **probe passed**. The Chrome extension's network recorder labelled every preview request 503 including successful page loads, so status codes were not usable as evidence; the reload-and-verify steps stand in for them |
| prod | 2026-09-14 (`db push` from the user's terminal; only 012 offered, history intact) | — (dev probe passed the same day; no organizer-side probe on prod) | `dev` promoted to `main` the same day (fast-forward) so Vercel Production runs the matching code. Both DB password files under `~/.config/holigay/` should now be deleted and the passwords rotated |
